...
class Group:
    ...
    def __str__(self):
        return f"{self.team} {self.units} {self.hp}"
...
while True:
    ...
    for g in sorted(selcombat(imm, inf).keys(), key=lambda g: g.initiative, reverse=True):
        if g not in combat:
            continue

        dmg = damage(g, combat[g])
        combat[g].units = combat[g].units - (dmg // combat[g].hp)
...
g = imm if imm else inf
print(sum(map(lambda g: g.units, g)))
